!> Module defining forward-in-time centred-in-space (FTCS) method for 1D LAE.
module class_lae_1d_solver_ftcs
    use, intrinsic :: iso_fortran_env
    use abstract_lae_1d_solver
    implicit none

    private
    public :: t_lae_1d_solver_ftcs

    !> Class name
    character(*), parameter :: class_name = "t_lae_1d_solver_ftcs"

    !> FTCS method solver
    type, extends(t_abstract_lae_1d_solver) :: t_lae_1d_solver_ftcs
        private
    ...
    subroutine initialize(this, dt, dx, wave_speed)
        class(t_lae_1d_solver_ftcs), intent(inout) :: this
        !> Time step
    ...
    function evaluate_next(this) result(h)
        ...
        do i = this%i_start, this%i_end
            d_flux = 0.5d0 * this%c * (this%solution(i+1) - this%solution(i-1))
